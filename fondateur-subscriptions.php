...
$user = $pdo->prepare("SELECT id, email, role, org_id, is_founder, is_super_admin FROM users WHERE id = :id LIMIT 1");
$user->execute([':id' => $user_id]);
$user = $user->fetch(PDO::FETCH_ASSOC);

// [FIX] Contrôle élargi : accepte role='founder'/'super_admin' OU is_founder=1 OU is_super_admin=1
$has_access = $user && (
    in_array($user['role'] ?? '', ['founder', 'super_admin'], true)
...
if (!$has_access) {
    ...
    render_sidebar('fondateur-subscriptions');
    echo '<main class="main"><div class="card"><h1>🚫 Accès refusé</h1></div></main>';
    render_foot();
    exit;
}

$flash = $_SESSION['flash_fondateur_subs'] ?? null;
unset($_SESSION['flash_fondateur_subs']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'change_plan') {
            ...
            // Sync etat essai de l'org selon le nouveau plan (corrige conversion essai -> payant)
            $newIsTrial = (int)$pdo->query("SELECT is_trial FROM asso_plans WHERE id = ".(int)$plan_id)->fetchColumn();
            if ($newIsTrial) {
                $pdo->prepare("UPDATE organizations SET status='trial', trial_ends_at=COALESCE(:e, trial_ends_at) WHERE id=:o")
                    ->execute([':e' => $period_end, ':o' => $org_id]);
            } else {
                $pdo->prepare("UPDATE organizations SET status='active', trial_ends_at=NULL WHERE id=:o")
                    ->execute([':o' => $org_id]);
            }
            $_SESSION['flash_fondateur_subs'] = ['type' => 'success', 'msg' => "Plan changé pour l'organisation #{$org_id}"];
            ...
            $_SESSION['flash_fondateur_subs'] = ['type' => 'success', 'msg' => "Org #{$org_id} marquée 'overdue' — tolérance jusqu'au {$grace_end}"];
            ...
            $_SESSION['flash_fondateur_subs'] = ['type' => 'success', 'msg' => "Org #{$org_id} downgradée au plan Démarrage"];
        }
    } catch (Throwable $e) {
        $_SESSION['flash_fondateur_subs'] = ['type' => 'error', 'msg' => $e->getMessage()];
    }
    header('Location: /fondateur-subscriptions');
    exit;
}
...
<main class="main">
  <div class="page-head" style="margin-bottom:24px;">
    ...
    <a href="/fondateur-plans" class="btn">⚙️ Gérer les plans</a>
  </div>
  ...
  <div class="card" style="padding:14px 18px;margin-bottom:18px;">
    <form method="get" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
      ...
      <?php if ($q || $filter_plan || $filter_status): ?>
        <a href="/fondateur-subscriptions" style="color:#64748B;font-size:13px;">↺ Reset</a>
      <?php endif; ?>
    </form>
  </div>
