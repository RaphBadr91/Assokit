...
if (!$has_access) {
    ...
    render_sidebar('fondateur-plans');
    echo '<main class="main"><div class="card"><h1>🚫 Accès refusé</h1><p>Cette page est réservée au fondateur.</p></div></main>';
    render_foot();
    exit;
}

$flash = $_SESSION['flash_fondateur_plans'] ?? null;
unset($_SESSION['flash_fondateur_plans']);
    ...
    try {
        if ($action === 'create' || $action === 'update') {
            ...
            if ($action === 'create') {
                ...
                $st->execute($params);
                $_SESSION['flash_fondateur_plans'] = ['type' => 'success', 'msg' => "Plan '{$data['name']}' créé avec succès"];
            } else {
                ...
                $st = $pdo->prepare($sql);
                $st->execute($params);
                $_SESSION['flash_fondateur_plans'] = ['type' => 'success', 'msg' => "Plan '{$data['name']}' mis à jour"];
            }
            ...
            $_SESSION['flash_fondateur_plans'] = ['type' => 'success', 'msg' => 'Plan supprimé'];
        }
    } catch (Throwable $e) {
        $_SESSION['flash_fondateur_plans'] = ['type' => 'error', 'msg' => $e->getMessage()];
    }
    header('Location: /fondateur-plans');
    exit;
}
  ...
  <div class="card" id="plan-form">
    ...
    <form method="post" style="display:grid;gap:16px;">
      ...
      <div style="display:flex;gap:10px;">
        ...
        <?php if ($editing): ?>
          <a href="/fondateur-plans" class="btn">Annuler</a>
        <?php endif; ?>
      </div>
    </form>
  </div>
